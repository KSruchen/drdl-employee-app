...
function EmployeeForm() {
  ...
  const onSubmit = data => {
    alert("Employee data submitted! " + JSON.stringify(data));
  };

  const fromDateVal = watch("fromDate");

  // Enhanced error messages for strict official validation
  ...
  return (
    <form className="employee-form" onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <div>
        ...
        <input
          {...register("roleNumber", {
            ...
            pattern: {
              value: /^\d+$/,
              message: "Role Number must contain digits only"
            }
          })}
      ...
      <div>
        ...
        <input
          {...register("fromDate", {
            required: "From Date selection is required"
          })}
          type="date"
        />
